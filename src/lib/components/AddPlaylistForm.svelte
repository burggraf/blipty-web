...
<div class="flex min-h-[50vh] items-center justify-center">
	<div class="w-full max-w-md space-y-6 rounded-lg border bg-card p-6 shadow-sm">
		<div class="space-y-2 text-center">
			<h1 class="text-2xl font-semibold tracking-tight">Add IPTV Playlist</h1>
			<p class="text-sm text-muted-foreground">Enter your Xtream Codes playlist details below</p>
		</div>

		<form class="space-y-4" onsubmit={handleSubmit}>
			<div class="space-y-2">
				<Input type="text" placeholder="Playlist Name" bind:value={name} required />
			</div>
			<div class="space-y-2">
				<Input
					type="url"
